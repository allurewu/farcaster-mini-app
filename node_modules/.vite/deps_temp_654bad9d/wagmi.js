...
async function recoverPublicKey({ hash, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-WV6DGWDD.js");
  const signature_ = (() => {
...
function padRight2(value, size6) {
  return pad(value, { dir: "right", size: size6 });
}
...
var getVersion2 = () => `@wagmi/core@${version2}`;
...
function createConfig(parameters) {
  ...
  if (version2.startsWith(prefix))
    currentVersion = Number.parseInt(version2.replace(prefix, ""));
  else
    currentVersion = Number.parseInt(version2.split(".")[0] ?? "0");
  const store = createStore2(subscribeWithSelector(
    // only use persist middleware if storage exists
    storage ? persist(getInitialState, {
      migrate(persistedState, version4) {
        if (version4 === currentVersion)
...
var version3 = "2.14.16";

// node_modules/wagmi/dist/esm/utils/getVersion.js
var getVersion3 = () => `wagmi@${version3}`;
